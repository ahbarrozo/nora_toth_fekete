...
works.get('/', async (c) => {
	...
	const whereClause: string = query
		? `WHERE wk.locale = '${query}'`
		: '';
	try {
		const result = await pool.query(`
            SELECT 
                wk.id, wk.date, wk.title, wk.link, 
				wk.description, wk.locale,    
            ...
            LEFT JOIN
                images im ON wki.image_id = im.id ${andClause}
			${whereClause}
			ORDER BY 
				wk.date
			DESC;
		...
		const works = result.rows.reduce((rows, row) => {
			...
			if (existingWork) {
				if (image) existingWork.images.push(image);
			} else {
				const work: Work = {
					id: row.id,
					date: row.date,
				...
				if (image) work.images.push(image);
...
works.post('/', authMiddleware, async (c) => {
	...
	try {
		...
		const workQuery = await pool.query(`
            INSERT INTO 
                works (date, link, title, description, locale)
			VALUES
				($1, $2, $3, $4, $5)
            RETURNING 
                id;`,
			[work.date, work.link, work.title, work.description, work.locale]
		);
...
works.put('/:id', authMiddleware, async (c) => {
	...
	try {
		...
		await pool.query(`
            ...
            WHERE 
                id = $4;`,
			[work.date, work.link, work.title, id]
		);
